...
Willard-Chandler Performance Notes
----------------------------------

The Willard-Chandler interface determination now ships with a compiled
implementation of the Gaussian KDE evaluation that uses pybind11 and OpenMP.
Key points to be aware of when you build pytim from source:

* A C++17 compiler with OpenMP support (``gcc``/``clang``/MSVC) and the
  ``pybind11`` headers are required at install time; these dependencies are
  pulled automatically by ``pip install .`` or ``pip install -e .``.
* The extension performs its own neighbour search in C++; no SciPy
  ``cKDTree`` queries are needed in the fast path.  If compilation fails or the
  module is missing, pytim seamlessly falls back to the original Python
  implementation.
* Threading is controlled via standard OpenMP environment variables.  For
  example ``OMP_NUM_THREADS=16`` enables 16-way parallel accumulation.  Set
  ``OMP_PROC_BIND``/``OMP_PLACES`` if you need finer control of thread pinning.
* Experimental CuPy support can be enabled by instantiating the interface with
  ``surface_backend='cupy'`` (and optionally
  ``surface_backend_options={'chunk_size': ...}``).  The fallback remains the
  optimised CPU path if CuPy is not available.
* You can verify the speed-up (and guard against regressions) by running the
  bundled benchmark::

      python benchmarks/willard_chandler_kde.py --points 2000 --grid 32768 --repeat 3 --min-speedup 5.0

  which fails if the accelerated path drops below the requested minimum
  speed-up.
